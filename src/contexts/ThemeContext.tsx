...
type ThemeRounding = 'sharp' | 'medium' | 'hz-r-md' | 'pill';
...
export const ThemeProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [rounding, setRounding] = useState<ThemeRounding>('hz-r-md');
  ...
  /* These getters name a ROLE from src/styles/system.css. They must never
     return a framework utility string — nothing in this repo compiles one. */
  const getRoundingClass = (): string => {
    switch (rounding) {
      case 'sharp': return 'hz-r-none';
      case 'medium': return 'hz-r-md';
      case 'hz-r-md': return 'hz-r-xl';
      case 'pill': return 'hz-r-full';
      default: return 'hz-r-xl';
    }
  };
  ...
  const getSpacingClass = (): string => {
    switch (spacing) {
      ...
      case 'spacious': return 'hz-p-6 hz-gap-6';
  ...
  const getFontClass = (): string => {
    switch (fontFamily) {
      case 'monospace': return 'hz-mono';
      default: return '';
    }
  };

  const getGlassClass = (): string => 'hz-glass hz-bordered';
